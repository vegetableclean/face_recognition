    ...
    for dir_item in os.listdir(path_name):
        ...
        if os.path.isdir(full_path):    #如果是文件夹，继续递归调用
            ...
            if dir_item.endswith('.jpg'):
                ...
                #print(image)  #確認是否讀檔成功
                #image = resize_image(gray_img, IMAGE_SIZE, IMAGE_SIZE)
                gray_img = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY) #將資料庫照片都轉為灰階圖片
                image = resize_image(gray_img, IMAGE_SIZE, IMAGE_SIZE)
           #放开这个代码，可以看到resize_image()函数的实际调用效果
                #cv2.imwrite('1.jpg', image)
                #np.save("text.npy",image)
                #print(image)  #看resize之後的檔案狀態
                images.append(image)                
